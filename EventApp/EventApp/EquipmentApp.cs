...
namespace EventApp
{
    public partial class EquipmentApp : Form
    {
            ...
            if (listBox_Hire.Items.Count != 0)
            {
                ...
                if (ShowInputDialog(ref braceletid) == DialogResult.OK)
                {
                    ...
                    if (dh.InsertEquipmentinHiredDB(Order, braceletid) == true)
                    {
                        ...
                        Order = new List<Equipment>();
                        MessageBox.Show("Equipment successfully hired!");
                    }
                    else
                    {
        ...
        public void AddToListBox()
        {
            ...
            {
                listBox_Hire.Items.Add(equip.Name.PadRight(20 - equip.Name.Length) + "\tQuantity: " + equip.Quantity.ToString().PadRight(2) + "\tPer day: " + equip.Price * equip.Quantity + " \u20AC");
            }
        ...
        private void LabelUpdate()
        {
            ...
            if (ListfromDB[8].Quantity == 0)
            {
                label4.ForeColor = Color.Red;
            }
            label4.Text = ListfromDB[8].Quantity.ToString();
            if (ListfromDB[7].Quantity == 0)
            {
                label7.ForeColor = Color.Red;
            }
            label7.Text = ListfromDB[7].Quantity.ToString();
            
        }

        private void hiredApp_FormClosed(object sender, FormClosedEventArgs e)
        {
            if (rfidhiredApp.Attached == true)
            {
